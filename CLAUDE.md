...
**挂机修仙宗门经营 + 深度随机化** 仙侠 Web 游戏。

## 方向更新（2026-04-10）

项目主方向已统一为 **挂机修仙**：

- 玩家负责定宗门方向、弟子分工、挂机远征模板和关键突破 / 飞升时机
- 宗门负责自动产出、闭关、远征、恢复、事件与长期成长
- 秘境仍保留随机性与构筑感，但默认服务于自动远征，不再作为高频手动主循环
- 失败默认表现为伤病、效率下降、收益波动与节奏回退，而不是常态化永久损失
- 离线报告、瓶颈诊断、宗门变化与自动化策略，优先级高于局内微操

当前方向基线见 `.impeccable.md`，完整重定稿见 `docs/superpowers/specs/2026-04-10-idle-cultivation-product-realignment.md`。
...
### 挂机经营层
- **自动修炼**：弟子每秒自动获取修为，灵石/灵气/灵药资源自动产出
- **自动突破**：满足条件自动触发境界突破（含失败风险与死亡）
- **离线收益**：关闭浏览器后持续计算（最长 24 小时），回来弹出离线报告
- **自动招募/自动跑秘境**：可开启自动化，完全挂机

### 自动远征层
- **11 个秘境**：按境界解锁，每局 5-8 层，每层随机路线与事件
- **随机掉落装备**：装备有品质（凡品~仙品）、随机词条（affix）、强化等级、6 种套装效果
- **随机技能与功法**：30 种功法按参悟度成长，16 种主动技能，秘境中随机发现
- **远征构建层**：每 2 层选祝福（13 种），通关获遗物（7 种），用于强化自动远征的差异化结果
- **装备自动穿戴**：出发前自动从仓库挑选最优装备，跑完自动归还
- **装备战斗加成**：装备属性（含套装加成）完整接入战斗计算

### 宗门策略层
- **宗门路线**：丹道/剑道/御兽三条路线，各含解锁节点
- **弟子价值判断**：弟子页展示 `留守价值 / 出战价值 / 承险能力`
- **秘境意图**：出发前以 `守成 / 争锋 / 寻机` 表达本局意图
- **战报归因**：战报归纳关键弟子、转折点与成败原因
- **宗门历史**：里程碑记录（首次招募/渡劫/boss 通关）

### 当前产品重心
- **核心**：宗门经营、弟子培养、自动远征、离线结算、飞升轮回
- **辅助**：局内手动肉鸽选择、逐层路线操作、细粒度战斗介入
- **目标**：把爽点集中到“配置正确后，整个宗门越转越顺”
...
2. 秘境页升级为自动远征配置页
3. 离线报告升级为情绪结算页
4. 弟子分工、建筑调度、资源溢出保护与瓶颈诊断系统落地
5. 飞升奖励从“数值加成”逐步转向“玩法权力”

详见 `docs/superpowers/specs/2026-04-10-idle-cultivation-product-realignment.md`。

## 设计上下文

### 用户
仙侠挂机 / 放置游戏爱好者。熟悉修仙放置类手游的低频上线与长期积累节奏，希望上线时做少量高价值决策，下线后宗门继续成长。可以接受随机性，但不希望被高频微操和高惩罚持续打断。
...
挂机放置 + 深度随机化 仙侠 Web 游戏。弟子、装备、功法等核心实体采用深度随机化生成——每个弟子的五行亲和、成长倍率、天赋词缀都是独一无二的随机组合。玩家的操作集中在经营判断和冒险押注，核心乐趣来自发现和培养独特的个体。水墨风格 UI，移动端底部导航 + 桌面端侧边栏。

**Core Value:** 挂机经营 + 深度随机化。玩家用很少的操作完成经营判断，核心乐趣来自"发现每个弟子独特的随机组合"和"秘境收获的随机装备/功法"。
