...
const PATH_STYLE_PROFILES: Record<CultivationPath, CombatStyleProfile> = {
  none: {
    ...
    preferredElements: ['neutral', 'fire', 'metal'],
    preferredCategories: ['attack', 'support', 'defense', 'ultimate'],
  },
  sword: {
    styleId: 'sword-burst',
    styleName: '剑修爆发',
    summary: '偏好金火爆发与收割终结技，重视先手和致命一击。',
    preferredElements: ['metal', 'fire', 'neutral'],
  ...
  body: {
    ...
    preferredElements: ['water', 'neutral', 'wood'],
  ...
  alchemy: {
    ...
    preferredElements: ['wood', 'fire', 'water'],
  ...
  beast: {
    ...
    preferredElements: ['metal', 'neutral', 'fire'],
  ...
  formation: {
    ...
    preferredElements: ['water', 'metal', 'neutral'],
    preferredCategories: ['defense', 'attack', 'support', 'ultimate'],
  },
  void: {
    styleId: 'void-gambit',
    ...
    preferredElements: ['metal', 'fire', 'water'],
...
function inferTechniqueElements(learnedTechniques: string[]): Element[] {
  ...
  if (ids.has('leishen') || ids.has('leiyu')) result.push('metal')
  if (ids.has('fentian') || ids.has('lieyan')) result.push('fire')
  if (ids.has('xuanbing') || ids.has('taishang')) result.push('water')
