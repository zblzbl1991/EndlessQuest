...
describe('getActiveSynergies', () => {
  it('returns empty when no conditions met', () => {
    const buildings = [makeBuilding('mainHall', 1), makeBuilding('spiritField', 2), makeBuilding('alchemyFurnace', 2)]
    expect(getActiveSynergies(buildings)).toHaveLength(0)
  })

  it('activates 灵药之道 when conditions met', () => {
    const buildings = [makeBuilding('mainHall', 3), makeBuilding('spiritField', 3), makeBuilding('alchemyFurnace', 3)]
  ...
  it('activates 百炼成钢 when conditions met', () => {
    const buildings = [makeBuilding('mainHall', 3), makeBuilding('spiritMine', 3), makeBuilding('forge', 3)]
    const active = getActiveSynergies(buildings)
...
describe('getSynergyBonus', () => {
  it('returns multiplier for matching building', () => {
    ...
    expect(bonus).toBeCloseTo(1.2) // +20% from 灵药之道
  })

  it('returns 1.0 when no synergy for building', () => {
    const buildings = [makeBuilding('mainHall', 1), makeBuilding('spiritMine', 1)]
  ...
  it('丹器双修 also gives bonus to forge', () => {
    const buildings = [makeBuilding('mainHall', 5), makeBuilding('alchemyFurnace', 5), makeBuilding('forge', 5)]
  ...
  it('market synergy returns 1.0 multiplier (quality cap handled separately)', () => {
    const buildings = [makeBuilding('mainHall', 5), makeBuilding('spiritMine', 5), makeBuilding('market', 3)]
